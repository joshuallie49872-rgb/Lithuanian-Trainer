...
function getNativeLang() {
  return (nativeLang || localStorage.getItem(LS.nativeLang) || "en").toLowerCase();
}
function getTargetLang() {
  return (targetLang || localStorage.getItem(LS.targetLang) || "lt").toLowerCase();
}

// Ensure manifest/course is loaded before anything tries to use manifest.lessons
async function ensureCourseLoaded() {
  if (manifest && Array.isArray(manifest.lessons) && manifest.lessons.length) return manifest;
...
----------------------------- */
function renderMap() {
  // If course data is not loaded yet, load it then re-render (prevents manifest.lessons undefined)
...
async function init() {
  try {
    // Bind all UI handlers first
    wireEvents();

    // Load selected languages into selects without reloading
    if (DOM.nativeLangSelect) DOM.nativeLangSelect.value = getNativeLang();
    if (DOM.targetLangSelect) DOM.targetLangSelect.value = getTargetLang();

    // Load course/manifest once up front so map/start/continue always has lessons[]
    await ensureCourseLoaded();
